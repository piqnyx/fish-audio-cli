...
_Validation:_ String value. The configuration validator currently performs no URL format
check; an unusable value fails when an HTTP request is made.
...
The `llm` section configures future LLM-backed text processors.

The passthrough processor does not use these settings.

### `llm.enabled`

Enables LLM-backed processing.
...
_Validation:_ String value. The configuration validator currently performs no URL format
check. This setting is used only when `llm.enabled` is `true`.

### `llm.model`

Model identifier sent to the configured endpoint.
...
_Validation:_ Must be a non-empty string when `llm.enabled` is `true`.

### `llm.timeoutSeconds`

Maximum duration of one LLM request.
...
_Validation:_ Must be an integer greater than zero when `llm.enabled` is `true`.
...
_Validation:_ Must be between `0.0` and `2.0`, inclusive, when the LLM is enabled.

### `llm.topP`

LLM nucleus sampling value.
...
_Validation:_ Must be between `0.0` and `1.0`, inclusive, when the LLM is enabled.

### `llm.maxOutputTokens`

Maximum number of tokens generated by the LLM response.
...
This limit applies to generated output, not input text.

_Validation:_ Must be an integer greater than zero when `llm.enabled` is `true`.

### `llm.jsonMode`

Requests structured JSON output from compatible providers.
...
API keys are read from separate files and are never stored directly in the JSON configuration.
...
### `secrets.llmApiKeyFile`

Path to the LLM provider API key file.
...
Missing secret files are created automatically with mode `0600`.

Secret directories should use mode `0700`.

Each secret file should contain only the API key and an optional trailing newline.

_Validation:_ Must be a non-empty path string when `llm.enabled` is `true`.
